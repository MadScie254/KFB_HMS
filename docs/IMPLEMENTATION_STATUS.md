...
Corrections follow the engineering audits in `AUDIT_2026-09-18.md`, which found three rows claiming behaviour the code did not have, and `AUDIT_2026-09-19_PRODUCTION.md`, which found fifteen defects that would have surfaced in a real hospital rather than a demonstration — including a production deployment that served no stylesheet and no JavaScript at all. The defects are fixed and covered by regression tests; the wording is corrected here so the table matches the software.
...
|---|---|---|
| Design system and responsive shell | Working | Icon-led role navigation, desktop/mobile layout, repeatable-line forms, accessible focus states, print styles and local assets only. Dark theme, compact-row density and sortable tables are remembered per person; a command palette (`/` or Ctrl+K) searches patients, visits, batches, orders and deliveries within what the caller's role may already open, which is covered by tests asserting the denials. Wide tables stack into labelled cards on a phone rather than hiding their right-hand columns behind an undiscoverable swipe, so the queue's status and waiting columns are readable on the device the owner actually uses. Waiting times escalate in colour, row tint and words. Figures are grouped (KES 1,540.00) and a grouped figure pasted back into a form is accepted. The browser is told when the connection drops or the session is about to end, and the six irreversible actions confirm once before posting. Every behaviour is an enhancement: with JavaScript unavailable the pages render, submit and navigate as before. See `UX_2026-09-20.md`. The owner and reviewer could not reach the stock ledger until September 2026 although the view already permitted them, and the dashboard computed a low-stock list it never rendered; both are corrected. |
| Authentication and permissions | Working core | Named Django accounts, server-side role decorators, session expiry, enforced lock screen, and sign-in throttling per username **and** per source address, so an attacker cannot spray a whole staff list from one machine without tripping anything. Secure cookies, HTTPS redirect and HSTS are on by default outside demo mode, and a weak `KFB_SECRET_KEY` or `KFB_DEBUG=1` is a startup failure rather than a warning. The lock screen was a no-op until the September 2026 audit (audit C1); enforcement now runs in `process_view` and is tested. Second factor and additional authentication for remote owner access remain production work. |
| Database and migrations | Working | Django migrations; SQLite is demo-only, PostgreSQL selected by `KFB_DATABASE_URL` for production. PostgreSQL receives an append-only trigger for audit events. The movement ledger, price versions, payments, receipts, custody and write-offs carry indexes on the columns every balance and statistic aggregates over; persistent connections are health-checked so a closed socket is not handed to the next request. |
| Static asset delivery | Working | WhiteNoise serves the collected static files with hashed names and immutable caching. Until 19 September 2026 nothing served them: Django does not with `DEBUG` off and the Caddy configuration proxies every path to the application, so a production deployment answered 404 for its own stylesheet, script and icon. `collectstatic` is a deploy step and `check_readiness` reports when it has not been run. |
...
| Pharmacy and stock ledger | Working primary flows | Multi-line outpatient prescriptions, walk-in pricing, FEFO batches, expiry/quarantine block and immutable movements. Negative stock was reachable through repeated product lines on one order until audit C2, and again through repeated batch lines on one ward issue until the 19 September production audit; both paths now reserve across lines and are tested. FEFO batch order is stated explicitly rather than left to the database, which sorted undated batches first on SQLite and last on PostgreSQL — the same order taking a different batch off the shelf in demo than in production. Stock now moves in both directions: deliveries post receipt movements, approved counts post adjustments, and ward issues move custody. Prescription replacement remains incomplete. |
...
| Stock statistics | Working core | Products at or below reorder level are drawn from the catalogue, not from stock rows: a product that had never been received produced no row and so never appeared, which meant the reorder list was silently missing exactly the items most urgently needing reorder. Stock-outs are reported separately from being below a threshold, and a product with no reorder level set is no longer reported as short by nothing. Valuation of sellable stock at batch purchase cost and at approved selling price, with expired and quarantined value reported separately rather than counted as an asset, products at or below reorder level, near-expiry/expired/quarantined batches, period receipts, units dispensed, cost of goods dispensed, product sales and gross margin, and most-dispensed products. Retail valuation states how many held products carry no approved price rather than understating the total silently. Product gross margin reports as unavailable until the period holds both a billed sale and a dispense. Stock discrepancy comes only from an approved count, never from an estimate. |
...
| Independent approvals | Working | Purchase orders, credit notes, M-PESA verification, stock counts and stock write-offs reject self-approval and have complete reviewer interfaces. A write-off is re-checked against the balance at approval, not only when proposed, because stock keeps moving while it waits for a reviewer. Ready eye cases have an accountable completion action. |
...
The suite is 148 tests, combining the audit regressions, PDF exports, protected attachments, multi-line forms, reviewer interfaces, requester segregation and all four CSV import paths with the stock-control, custody, write-off, valuation-correctness, intelligence and continuous-integration cases. Automated tests cover the core of scenarios 1-9, 12 and 14, including clinician prescription to pharmacy pricing to reception payment to actual dispense, and scenario 3 end to end through a recorded delivery. Scenarios 11 and 15-20 need further workflow screens, infrastructure or supervised operational testing. Desktop and 390 x 844 phone layouts were inspected in the live local browser during delivery; the phone document width stayed within its viewport and the browser console remained clean. The 19 September production audit drove 216 page loads across all nine roles with zero server errors, and verified static delivery with `DEBUG=False` after running `collectstatic`.
